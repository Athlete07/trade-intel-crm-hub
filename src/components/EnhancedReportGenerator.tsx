...
export function EnhancedReportGenerator({ onBack }: EnhancedReportGeneratorProps) {
  ...
  const fetchReportData = async () => {
    try {
      ...
      const [companiesRes, dealsRes, contactsRes, tasksRes, documentsRes] = await Promise.all([
...
- Average Contacts per Company: ${data.companies.length > 0 ? (data.contacts.length / data.companies.length).toFixed(1) : 0}

COMPANY BREAKDOWN:
${data.companies.slice(0, 10).map((company: any, index: number) => 
  `${index + 1}. ${company.name || 'Unknown Company'} - ${company.type || 'Unknown Type'} (${company.country || 'Unknown Country'})`
).join('\n')}`;
    }
...
APPENDIX
--------
Data Sources: Companies, Deals, Contacts, Tasks, Documents
